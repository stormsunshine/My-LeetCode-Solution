...
## Solution
Use a `Map` which maps integers to sets to record the previous jump lengths to reach each stone. Initially, the first stone at position 0 corresponds to a set that contains 0 only. For each stone, check which stones can be reached from the current stone, and update the next stones' jump lengths. If a stone can be reached, then it will be a key in the `Map`.

Finally, check whether the last stone is a key in the `Map`, and return `true` if and only if the last stone is a key in the `Map`.
