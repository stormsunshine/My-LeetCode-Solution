...
Step 1: Reformat the expression string. Remove all the empty spaces, and if a minus sign `-` is just after an open parentheses `(`, then add a number zero `0` between them. Adding zeroes is not necessary in this problem since all integers in the expression string are non-negative, but it is a good practice to add zeroes.
