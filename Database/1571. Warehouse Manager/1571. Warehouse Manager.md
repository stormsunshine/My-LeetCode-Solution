...
## Solution
The volume of each product is calculated as `units * Width * Length * Height`. For each warehouse, join tables `Warehouse` and `Products` using the field `product_id` and calculate the sum of all products' volumes. Finally, group the result by `warehouse_name`.
```
