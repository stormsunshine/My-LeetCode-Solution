...
## Solution
For each month, count the number of distinct orders and the number of distinct customers where `invoice > 20`. Use `date_format` to convert the date into the required format. Finally, group the result by the year and the month of `order_date`.
```
