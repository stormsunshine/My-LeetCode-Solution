...
## Solution
Select the entries with `salary < 30000` and `manager_id` do not appear as a value of `employee_id`. Keep the values `employee_id` for these entries and order by `employee_id`.
```
