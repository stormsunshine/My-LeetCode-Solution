...
## Solution
Use `dayofweek` to obtain the day of the week of a date. Use `case` statements to obtain the quantities of the category items. Join table `Items` and table `Orders` to obtain the quantity of each `order_id` and `customer_id`.
```
