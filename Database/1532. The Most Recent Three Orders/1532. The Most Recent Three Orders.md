...
## Solution
Join `Orders` with itself, and for each customer, select the orders such that for each order, there are at most 3 orders after the order. Finally, sort the result by customer's name, customer's id and order date accordingly.
```
